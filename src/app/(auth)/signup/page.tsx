"use client";

import { useState } from "react";
import Link from "next/link";
import { useRouter } from "next/navigation";
import { createClient } from "@/lib/supabase/client";
import { useToast } from "@/hooks/useToast";
import { signup } from "@/lib/actions/auth";
import { CREATOR_CATEGORIES, SOCIAL_REACH_OPTIONS } from "@/lib/constants/auth";
import type { AccountType, CreatorCategory, SocialReach } from "@/types/auth";
import s from "../auth.module.css";

export default function SignupPage() {
  const [accountType, setAccountType] = useState<AccountType>("subscriber");
  const [nickname, setNickname] = useState("");
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [category, setCategory] = useState<CreatorCategory>("general");
  const [socialReach, setSocialReach] = useState<SocialReach>("100");
  const [isLoading, setIsLoading] = useState(false);
  const { showToast } = useToast();
  const router = useRouter();

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);
    const result = await signup({
      email, password, nickname, accountType,
      ...(accountType === "creator" && { category, socialReach }),
    });
    if (result.success) {
      router.push("/confirm");
    } else {
      showToast(result.message, "error");
    }
    setIsLoading(false);
  };

  const handleSocialLogin = async (provider: "google" | "apple") => {
    const supabase = createClient();
    const { error } = await supabase.auth.signInWithOAuth({
      provider,
      options: { redirectTo: `${window.location.origin}/auth/callback` },
    });
    if (error) showToast(error.message, "error");
  };

  return (
    <>
      {/* Card */}
      <div className={s.authCard}>
        <h1>Create an account</h1>
        <p className={s.subtitle}>Join Vaulty and start in minutes.</p>

        {/* Account type selector */}
        <div className={s.typeSelector}>
          <button
            type="button"
            className={accountType === "subscriber" ? s.typeOptActive : s.typeOpt}
            onClick={() => setAccountType("subscriber")}
          >
            <span className={s.typeLabel}>Subscriber</span>
            <span className={s.typeDesc}>Support creators</span>
          </button>
          <button
            type="button"
            className={accountType === "creator" ? s.typeOptActive : s.typeOpt}
            onClick={() => setAccountType("creator")}
          >
            <span className={s.typeLabel}>Creator</span>
            <span className={s.typeDesc}>Sell your content</span>
          </button>
        </div>

        <form onSubmit={handleSubmit}>
          <div className={s.formGroup}>
            <label>Nickname</label>
            <input
              type="text"
              className={s.formInput}
              placeholder="How others will see you"
              value={nickname}
              onChange={(e) => setNickname(e.target.value)}
              required
            />
          </div>
          <div className={s.formGroup}>
            <label>Email</label>
            <input
              type="email"
              className={s.formInput}
              placeholder="john@example.com"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              required
            />
          </div>
          <div className={s.formGroup}>
            <label>Password</label>
            <input
              type="password"
              className={s.formInput}
              placeholder="Min. 8 characters"
              value={password}
              onChange={(e) => setPassword(e.target.value)}
              required
              minLength={8}
            />
          </div>

          {/* Creator-only fields */}
          {accountType === "creator" && (
            <>
              <label className={s.fieldLabel}>Category</label>
              <div className={s.catSelector}>
                {CREATOR_CATEGORIES.map((cat) => (
                  <button
                    key={cat.value}
                    type="button"
                    className={category === cat.value ? s.catOptActive : s.catOpt}
                    onClick={() => setCategory(cat.value as CreatorCategory)}
                  >
                    <span className={s.typeLabel}>{cat.label}</span>
                    <span className={s.typeDesc}>{cat.value === "general" ? "All audiences" : "Adult content"}</span>
                  </button>
                ))}
              </div>

              <label className={s.fieldLabel}>Your social reach</label>
              <div className={s.socialReach}>
                {SOCIAL_REACH_OPTIONS.map((opt) => (
                  <button
                    key={opt.value}
                    type="button"
                    className={socialReach === opt.value ? s.reachBoxActive : s.reachBox}
                    onClick={() => setSocialReach(opt.value as SocialReach)}
                  >
                    <span className={s.reachName}>{opt.label}</span>
                    <span className={s.reachSub}>{opt.sub}</span>
                  </button>
                ))}
              </div>
            </>
          )}

          <button
            type="submit"
            className={s.authSubmitMargin}
            disabled={isLoading}
          >
            {isLoading && (
              <svg className={s.spinner} viewBox="0 0 24 24" fill="none">
                <circle opacity={0.25} cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                <path opacity={0.75} fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
              </svg>
            )}
            {accountType === "creator" ? "Create creator account" : "Create account"}
          </button>
        </form>

        <div className={s.authDivider}><span className={s.dividerText}>or continue with</span></div>

        <div className={s.socialBtns}>
          <button type="button" className={s.socialBtn} onClick={() => handleSocialLogin("google")}>
            <svg viewBox="0 0 24 24" fill="currentColor">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4" />
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
            </svg>
            Google
          </button>
          <button type="button" className={s.socialBtn} onClick={() => handleSocialLogin("apple")}>
            <svg viewBox="0 0 24 24" fill="currentColor">
              <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z" />
            </svg>
            Apple
          </button>
        </div>
      </div>

      {/* Footer */}
      <div className={s.authFooter}>
        Already have an account? <Link href="/login">Log in</Link>
      </div>

    </>
  );
}
